...
function Row($data)
   ...
   $this->CheckPageBreak($h);
	...
	function Header()
	{
	    ...
	    $this->Text(95,30,'Cooperativas',0,'C', 0);
    $this->Ln(13);
	}
...
// creamos el objeto FPDF
	...
	$pdf->SetMargins(30,10,10); // definimos los margenes en este caso estan en milimetros
...
// Para realizar esto utilizaremos la funcion Row()
	...
	$pdf->SetWidths(array(50,25,55,25,18)); // Definimos el tamaño de las columnas, tomando en cuenta que las declaramos en milimetros, ya que nuestra hoja esta en milimetro15s
	$pdf->SetDrawColor(210,0,0);
	$pdf->Row(array('Nombres de la Cooperativa', 'Rif', 'RSIP', 'Municipio', 'Cantidad miembros')); // creamos nuestra fila con las columnas fecha(fecha de la visita al medico), medico(nombre del medico que nos atendio), consultorio y el diagnostico en esa visita
	$pdf->SetFont('Times','',9);
	$pdf->SetDrawColor(210,0,0);
 ...
 // Realizamos nuestra consulta
	$strConsulta2 = "SELECT * FROM cooperatva group by red";
	...
	$strConsulta4 = "SELECT count(cedula) as cant FROM miembrosc where cooperativa='$fila[rif]'";
	...
	$pdf->Row(array($fila['nombre'], $fila['rif'], $fila2['nombre'], $fila['muncipip'], $fila3['cant']));
	    }
