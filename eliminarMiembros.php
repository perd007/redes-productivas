...
$sql="delete from miembrosc where cedula=$id";
$verificar=mysql_query($sql,$conexion) or die(mysql_error());

if($verificar){
	echo"<script type=\"text/javascript\">alert ('Datos Eliminado'); location.href='sistemenus.php?valor=c51&link=link3&cooperativa=$cooperativa' </script>";
}
else
	echo"<script type=\"text/javascript\">alert ('Error'); location.href='sistemenus.php?valor=c51&link=link3&cooperativa=$cooperativa'</script>";
