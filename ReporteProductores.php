...
function Row($data)
   ...
   $this->CheckPageBreak($h);
	...
	function Header()
	{
	    ...
	    $this->Text(75,30,'Productores Independientes',0,'C', 0);
    $this->Ln(13);
	}
...
// Para realizar esto utilizaremos la funcion Row()
	...
	$pdf->SetWidths(array(25,25,15,25,45,18,40)); // Definimos el tamaño de las columnas, tomando en cuenta que las declaramos en milimetros, ya que nuestra hoja esta en milimetro15s
	$pdf->SetDrawColor(210,0,0);
	$pdf->Row(array('Nombres', 'Apellidos', 'cedula', 'RSIP', 'Direccion', 'Telefono', 'Correo')); // creamos nuestra fila con las columnas fecha(fecha de la visita al medico), medico(nombre del medico que nos atendio), consultorio y el diagnostico en esa visita
	$pdf->SetFont('Times','',9);
	$pdf->SetDrawColor(210,0,0);
 ...
 // Realizamos nuestra consulta
	$strConsulta2 = "SELECT * FROM productores order by id_red asc";
	...
	$strConsulta3 = "SELECT * FROM redes where id='$fila[id_red]'";
	...
	$pdf->Row(array($fila['nombre'], $fila['apellido'], $fila['cedula'], $fila2['nombre'], $fila['direccion'], $fila['telefono'], $fila['correo']));
	    }
