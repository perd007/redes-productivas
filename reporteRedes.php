...
// Para realizar esto utilizaremos la funcion Row()
	...
	$pdf->SetWidths(array(30,40,18,40,22,50)); // Definimos el tamaño de las columnas, tomando en cuenta que las declaramos en milimetros, ya que nuestra hoja esta en milimetro15s
	$pdf->SetDrawColor(210,0,0);
	$pdf->Row(array('Nombre', 'Representante', 'cedula', 'Direccion', 'Telefono', 'Correo')); // creamos nuestra fila con las columnas fecha(fecha de la visita al medico), medico(nombre del medico que nos atendio), consultorio y el diagnostico en esa visita
	$pdf->SetFont('Times','',9);
	$pdf->SetDrawColor(210,0,0);
 ...
 // Realizamos nuestra consulta
	$strConsulta2 = "SELECT * FROM redes";
	...
	$cadena=$fila['nombre_rep']." ".$fila['apellido_rep'];
	// los mostramos con la función Row
	$pdf->Row(array($fila['nombre'], $cadena, $fila['cedula_rep'], $fila['direccion'], $fila['telefono'], $fila['correo']));
	    }
